...
bool FindClient(string AccountNum, string pincode, vector<stClient> vClients, stClient& client) {

    for (stClient& C : vClients) {

        if (AccountNum == C.AccountNum && pincode == C.PinCode) {
...
void QuickWithdraw(short amount) {
    ...
    char answer;
    cout << "\n Are you sure to withdraw amount [ " << abs(amount) << " ] ? (y/n)";
    cin >> answer;
...
void NormalWithdraw() {
    ...
    cout << "\n enter amount to withdraw divisible by 5: ";
    cin >> amount;
    if (amount % 5 != 0) {
...
void LoginMenu() {
    ...
    while (!FindClient(AccountNum, PinCode, vClients, Client)) {
        LoginScreen();
        cout << "\n faild account number or pincode..  ";
        ReadClientInfo(AccountNum, PinCode);
    }
}
